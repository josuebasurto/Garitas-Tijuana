...
//RSS SanYsidro-Otay-Tecate http://apps.cbp.gov/bwt/customize_rss.asp?portList=250401,250601,250501&lane=all&action=rss&f=html
//RSS Clima Tijuana http://weather.yahooapis.com/forecastrss?w=149361&u=c
    $.ajax({
        type: 'POST',
        url: 'http://apps.cbp.gov/bwt/customize_rss.asp?portList=250401,250601,250501&lane=all&action=rss&f=html',
        cache: false,

        beforeSend:function(){
            //Loading image
            $('#Garitas').html('<div class="loading" style="text-align:center"><img src="imagenes/loading.gif" alt="Cargando..." /></div>');
        },
        success:function(data){
            //Successful request
            $('#Garitas').empty();
            $(data).find('item').each(function(i){
                $('#Garitas').append(tiempo($(this).find('title').text() , $(this).text()));
            });
        },
        error:function(){
            //Failed request
            $('#Garitas').html('<ul data-role="listview" data-inset="true" id="ListaSanYsidro" ><li data-theme="b"><div class="ui-btn-text" ><a href="index.html" class="ui-link-inherit">No Hay Informacion</a></div></li></ul>');
        }
    });
